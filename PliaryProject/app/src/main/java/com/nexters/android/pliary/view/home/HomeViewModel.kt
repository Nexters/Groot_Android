...
class HomeViewModel @Inject constructor() : BaseViewModel() {
    ...
    // 리스트 초기화
    private val _listSetData = MutableLiveData<ArrayList<PlantCard>>()
    val listSetData: LiveData<ArrayList<PlantCard>> get() = _listSetData

    // 새 카드 등록
    private val _addCardEvent = MutableLiveData<Unit>()
    val addCardEvent: LiveData<Unit> get() = _addCardEvent

    fun reqPlantCardData() {
        _listSetData.value = arrayListOf<PlantCard>(
